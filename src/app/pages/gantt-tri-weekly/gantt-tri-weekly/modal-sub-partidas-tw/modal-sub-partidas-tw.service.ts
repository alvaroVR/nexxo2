...
import {ModalSubPartidasTwComponent} from "./modal-sub-partidas-tw.component";
import {ModalRealizadaComponent} from "../modal-realizada/modal-realizada.component";

@Injectable({
  providedIn: 'root'
})
export class ModalSubPartidasTwService {
  ...
  public alerta(title: string, message: string, data: any): Observable<string> {
    ...
    this.bsModalRef = this.bsModalService.show(ModalSubPartidasTwComponent, {
      initialState,
      class: 'modal-lg',
